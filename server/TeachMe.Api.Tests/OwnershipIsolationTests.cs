...
public class OwnershipIsolationTests : IClassFixture<ApiFactory>
{
    ...
    /// <summary>
    /// Teacher to lessons is one-to-many, and this is the test that says so rather than leaving it
    /// to be inferred from the schema. A teacher holds as many lessons as they publish; a lesson
    /// belongs to exactly one teacher, which is why the second teacher's list is not merely
    /// missing the first teacher's lessons but is a wholly separate set — and why the two may both
    /// use `orderIndex` 1 without colliding, the unique index being over the pair
    /// (`TeacherUserId`, `OrderIndex`) and not over the order alone.
    /// </summary>
    [Fact]
    public async Task A_teacher_holds_many_lessons_and_each_lesson_holds_one_teacher()
    {
        var admin = await TestAuth.SignedInAdminAsync(_factory);

        var teacherA = await TestAuth.RegisterAndSignInTeacherAsync(_factory, "iso1n.A@test.local");
        await ApproveAsync(admin, "iso1n.A@test.local");
        var teacherB = await TestAuth.RegisterAndSignInTeacherAsync(_factory, "iso1n.B@test.local");
        await ApproveAsync(admin, "iso1n.B@test.local");

        // The many side: one teacher, three lessons.
        var first = await CreateLessonAsync(teacherA, "A first", 1);
        var second = await CreateLessonAsync(teacherA, "A second", 2);
        var third = await CreateLessonAsync(teacherA, "A third", 3);

        // The one side: the same order index again, under a different teacher, is not a collision.
    ...
    private record PagedLessons(List<LessonRow> Items);

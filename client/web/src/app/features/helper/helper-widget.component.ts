      ...
      <div class="panel" role="dialog" aria-label="Helper">
        ...
        <div class="panel-body" aria-live="polite">
          @if (asking()) {
            ...
            <button mat-button color="primary" (click)="ask()">
              Try again
            </button>
          } @else {
            @if (result(); as r) {
              @if (r.unknown) {
                ...
                <ul class="topics">
                  @for (topic of r.knownTopics; track topic) {
                    <li>{{ topic }}</li>
                  }
                </ul>
              } @else {
                <p>{{ r.answer }}</p>
                @if (r.route) {
                  <button mat-flat-button color="primary" (click)="go(r.route)">
                    Take me there
                  </button>
                }
              }
              ...
              <p class="text-muted">
                Ask me something like "where are my results" or "how do I join a
                course".
              </p>
            }
          }
...
export class HelperWidgetComponent {
  ...
  ask(): void {
    ...
    this.http
      .get<HelperAnswer>(`/api/helper/ask?q=${encodeURIComponent(question)}`)
      .subscribe({
        next: (res) => {
          this.result.set(res);
        ...
        error: (err) => {
          ...
          this.failure.set(
            problemFrom(err).title ?? 'Something went wrong. Try again.',
          );
          this.asking.set(false);
        },
      });

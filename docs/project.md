...
The date of birth is the smallest of the seven, and the only one that changes a requirement's own screen rather than adding a new one. Req 6 names *"the parts they may change"* without listing them, so a fourth editable field is inside the brief rather than beyond it — and the line that matters is untouched: what may **not** be changed is still absent from the form *and* from the request.

...
_Added 2026-08-28, extended 2026-08-30. Everything above is the client's brief as given, unchanged. Everything below was built **after** all twenty-three requirements were passing, and is recorded here so that the brief and the thing that exists do not quietly disagree._
...
| **Lists that scroll instead of page**   | [`plan.md` §12.6](plan.md)   | Every list hands back a slice and a cursor, and asks for the next as the reader nears the foot of it. No pagers. |
| **Dark mode**                           | [`darkmode.md`](darkmode.md) | The same palette on a second ground, measured to the same 4.5:1 — light, dark, or whatever the reader's device says. |
| **Search and filters on every list**    | [`plan.md` §12.8](plan.md)   | One search box above every list, and a filter group where the rows have a state worth separating. Paged lists ask the server; whole ones narrow in place. |
...
**On lists that scroll.** This one adds no column, no endpoint and no screen — it changes how every list the brief already asked for is *read*. Req 23 names the three awkward states a list must have (loading, error, empty); it says nothing about what a list does when it is long, because on the data the brief imagined none of them were. They are now: a teacher's roster runs to forty-odd students, the directory to thirty-eight courses, the approvals queue to fifteen. So each list hands back one slice and a **cursor** — the sort key of the last row it gave you — and the next slice resumes from that row rather than from a count of rows.

The reason it is a cursor and not a page number is a correctness one, and it is the reason the change is in this addendum rather than in a styling note. Every one of these lists is read *while it is being written to*, and the approvals queue is the sharpest case: the administrator working down it is the person changing it. Skipping a **count** of rows would mean that approving the tenth teacher pushes the eleventh past a boundary unseen — a row silently never shown, on the one screen in the app whose entire job is that every waiting teacher gets looked at. Resuming from a row cannot do that.

Nothing a requirement promised has moved. Req 23's three states are still one component and still cover every list; a slice that fails *after* the first one keeps the rows already on screen rather than replacing them with an error, because those rows are still true. Req 20 and Req 21 are unaffected: a cursor is not a credential, and it is not treated as one — the ownership filter is applied from the cookie on **every** slice, so handing somebody your cursor hands them nothing. One thing did have to change to make room for it, and it is recorded rather than hidden: **reordering lessons** used to send the teacher's whole ordered list of lesson ids, which a screen holding only what has been scrolled to cannot state. It now sends one lesson and a direction, and the server finds the neighbour. Req 8's *"a place another lesson holds"* is still a 400 from the same unique index.

**On dark mode.** This one adds no column, no endpoint, no screen and no request — it adds a *second set of values for the tokens the theme already had*. The brief says nothing about a ground, and it did not need to: it names twenty-three outcomes and the ways they may fail, and none of them turn on which colour the page is. What it does say, in Req 23 and in the plan's §8, is that this platform's readers are real people using real devices, and the thing that made a second ground worth building is that **every other app on that device had already stopped choosing for them.**

It is on the device rather than on the account, and the reason is one of the brief's own requirements. Req 7 asks for a home page *before anyone signs in*, and the addendum added a directory read the same way. A preference stored against a user and fetched with the session cannot reach either — so the two screens a visitor sees first would be in the ground they did not choose, and would then snap when they signed in. A ground is a property of the room, not of the person: the same student reads in bed and in a lab. So it is three states in the browser — light, dark, or whatever the device says — and `Users` keeps its property of holding only who somebody is and what they may do.

**The claim §8 makes is duplicated, not relaxed.** Its palette table's whole value is that the contrast ratio beside each colour is *measured and written down*; the dark palette is measured the same way against the dark ground, and every colour that carries a word clears the same 4.5:1. The two bright ambers that §8 demoted to fills — because they failed that claim at 2.9:1 and 3.6:1 — do not move between grounds at all, which is the point: a second ground is exactly where a demotion like that would quietly lapse. And the check itself stopped being a promise: `contrast.mjs` recomputes both grounds and fails below 4.5:1. Pointed at the *existing* light theme it immediately found a real defect — the initials tile on a roster is white on `hsl(h 55% 42%)`, which at one of its twelve hues is 2.59:1 and had been failing since the day it was written, for whichever people's ids happened to hash to it.

**On search and filters.** This one adds no column, no endpoint and no screen either — it adds two optional query parameters to the lists that already existed, and one box above each. The brief could not have asked for it: on the data it imagined, every list fitted on a screen. On the data the demo now carries — forty-odd students on a roster, thirty-eight courses in the directory — *"find Sara"* had exactly one answer, which was to scroll until Sara appeared. The rule that governs it is the same one that governs the cursor above: **narrowing a list must never widen it**. Every search runs on the query that already carries the ownership filter, and on a student's course view the term is applied *before* the projection that decides what a student may see — so a lesson whose moment has not come cannot be searched into view, which is Req 19 restated rather than revisited. One thing did have to give: on a filtered lesson list the reorder arrows are **disabled**, because the row above is no longer the neighbour the server would swap with, and an arrow that lies is worse than an arrow that waits.

The date of birth is the smallest of them, and the only one that changes a requirement's own screen rather than adding a new one. Req 6 names *"the parts they may change"* without listing them, so a fourth editable field is inside the brief rather than beyond it — and the line that matters is untouched: what may **not** be changed is still absent from the form *and* from the request.
...
And one that takes a second and is worth the second: **flip the laptop's own appearance setting while the demo is on screen**, signed out, on the home page. The app follows without a reload and without anyone having opened a menu — because nobody chose the ground and the device already had an answer. The menu in the bar is then the override, and it holds against the OS in both directions.

Two more, on the two screens a teacher lives on: **type half a student's email into the box above the roster** — the match comes back from the server, not from the rows already drawn, so it finds somebody forty rows down who has never been scrolled to. Then on `/teacher/progress` pick **Not started**: the list is the students nobody has marked yet, which is the question that screen is opened to answer.

And one moment that is best shown rather than described: **scroll `/discover` to the bottom**. The demo database now carries thirty-eight courses, so there are six more slices under the first screen and they arrive as you reach them, with no pager and no jump. The same is true of the approvals queue and of either signed-in teacher's roster, which is why the seeded cohort is the size it is: a list that fits on one screen demonstrates this exactly as well as a list with no cursor at all.
